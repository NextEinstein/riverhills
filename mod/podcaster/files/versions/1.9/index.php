...
# HU, CS Repository plugin support
                                'link' => $ME."?id=$course->id&amp;wdir=/&amp;choose=$choose".$repository->get_arg (),
                                'type' => 'misc');
...
# HU, CS Repository plugin support
                                    'link' => $ME."?id=$course->id&amp;wdir=$link&amp;choose=$choose".$repository->get_arg (),
                                    'type' => 'misc');
            }
...
# HU, CS Repository plugin support
    $scheme = $repository->get_scheme ();
    if (! $basedir = repository_make_upload_directory($scheme.'://'.$course->id)) {
        error("The site administrator needs to fix the file permissions");
    }

    $baseweb = $CFG->wwwroot;
# HU, CS Repository plugin support
    // $basedir = $repository->get_scheme ().'://';
//  End of configuration and access control
    ...
    if ($wdir == "/backupdata") {
# HU, CS Repository plugin support
        if (! repositorymake_upload_directory($scheme.'://'.$course->id.'/backupdata')) {   // Backup folder
            error("Could not create backupdata folder.  The site administrator needs to fix the file permissions");
        }
    }

# HU, CS Repository plugin support
    if (!repository_is_dir($basedir.$wdir)) {
    ...
    switch ($action) {
        ...
        case "upload":
            html_header($course, $wdir);
# HU, CS Repository plugin support
            require_once($CFG->dirroot.'/mod/podcaster/files/versions/'.$filesversion.'/uploadlib.php');
             
            if ($save and confirm_sesskey()) {
                $course->maxbytes = 0;  // We are ignoring course limits
                $um = new upload_manager('userfile',false,false,$course,false,0);
# HU, CS Repository plugin support
                $um->repository = $repository;
                $dir = "$basedir$wdir";
...
# HU, CS Repository plugin support
                echo " <input type=\"hidden\" name=\"repository\" value=\"".$scheme."\" />";
                upload_print_form_fragment(1,array('userfile'),null,false,null,$upload_max_filesize,0,false);
                echo " </td></tr></table>";
                echo " <input type=\"submit\" name=\"save\" value=\"$struploadthisfile\" />";
# HU, CS Repository plugin support
                echo '<script type="text/javascript">validate_confirm_license ();</script>';
...
# HU, CS Repository plugin support
                require_once ($CFG->dirroot.'/mod/podcaster/lib.php');
                html_header($course, $wdir);
                if (!empty($USER->filelist)) {
                    foreach ($USER->filelist as $file) {
# HU, CS Repository plugin support
                        podcaster_sync_file ($repository->id, substr($file, 1), 'delete');
...
# HU, CS Repository plugin support
                        $optionsyes = array('id'=>$id, 'wdir'=>$wdir, 'action'=>'delete', 'confirm'=>1, 'sesskey'=>sesskey(), 'choose'=>$choose, 'repository' => $scheme);
                        $optionsno  = array('id'=>$id, 'wdir'=>$wdir, 'action'=>'cancel', 'choose'=>$choose, 'repository' => $scheme);
...
# HU, CS Repository plugin support
                if (!$repository->is_allowed ($name) && !$repository->is_folder ($basedir.$wdir.'/'.$oldname)) {
                  notify(get_string('mimetype_not_allowed','podcaster'));
                } elseif (repository_file_exists($basedir.$wdir."/".$name)) {
...
# HU, CS Repository plugin support
                } else if (! repository_make_upload_directory($scheme.'://'.$course->id.$wdir.'/'.$name)) {
...
# HU, CS Repository Plugin support
                                "index.php?id=$id&amp;wdir=$wdir&amp;action=cancel".$repository->get_arg ());
            } else {
                displaydir($wdir);
            }
            html_footer();
...
# HU, CS Repository plugin support
    global $basedir, $repository, $scheme;
    global $id;
    global $USER, $CFG;
    global $choose;
...
# HU, CS Repository plugin support
        if (repository_is_dir($fullpath."/".$file)) {
            $dirlist[] = $file;
        } else {
            $filelist[] = $file;
        }
...
# HU, CS Repository plugin support
                print_cell('left', '<a href="index.php?id='.$id.'&amp;wdir='.$fileurl.'&amp;choose='.$choose.$repository->get_arg ().'"><img src="'.$CFG->pixpath.'/f/parent.gif" class="icon" alt="" />&nbsp;'.get_string('parentfolder').'</a>', 'name');
                print_cell();
                print_cell();
                print_cell();
...
# HU, CS Repository plugin support
                $filesize = display_size(repository_get_directory_size("$fullpath/$dir"));
# HU, CS Repository plugin support
                $filedate = userdate(repository_filemtime($filename), "%d %b %Y, %I:%M %p");
                print_cell("center", "<input type=\"checkbox\" name=\"file$count\" value=\"$fileurl\" />", 'checkbox');
# HU, CS Repository plugin support
                print_cell("left", "<a href=\"index.php?id=$id&amp;wdir=$fileurl&amp;choose=$choose".$repository->get_arg()."\"><img src=\"$CFG->pixpath/f/folder.gif\" class=\"icon\" alt=\"$strfolder\" />&nbsp;".htmlspecialchars($dir)."</a>", 'name');
                print_cell("right", $filesize, 'size');
                print_cell("right", $filedate, 'date');
                print_cell("right", "<a href=\"index.php?id=$id&amp;wdir=$wdir&amp;file=$filesafe&amp;action=rename&amp;choose=$choose".$repository->get_arg ()."\">$strrename</a>", 'commands');
            }
...
# HU, CS Repository plugin support
            $ffurl = $repository->get_url ($fileurl, $id);

            repository_link_to_popup_window ($ffurl, "display",
                                  "<img src=\"$CFG->pixpath/f/$icon\" class=\"icon\" alt=\"$strfile\" />&nbsp;".htmlspecialchars($file),
                                  480, 640);
            echo "</td>";

# HU, CS Repository plugin support
            $file_size = repository_filesize($filename);
            ...
            if ($icon == "text.gif" || $icon == "html.gif") {
                $edittext .= "<a href=\"index.php?id=$id&amp;wdir=$wdir&amp;file=$fileurl&amp;action=edit&amp;choose=$choose".$repository->get_arg ()."\">$stredit</a>";
...
# HU, CS Repository plugin support
            ...
            print_cell("right", "$edittext <a href=\"index.php?id=$id&amp;wdir=$wdir&amp;file=$filesafe&amp;action=rename&amp;choose=$choose".$repository->get_arg ()."\">$strrename</a>", 'commands');
