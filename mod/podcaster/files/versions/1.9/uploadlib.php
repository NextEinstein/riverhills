 ...
 *
 * @author ?
 * @version $Id: uploadlib.php,v 1.25.4.2 2008/03/20 06:26:20 nicolasconnault Exp $
...
# HU, CS Repository plugin support
                if (!$this->repository->is_allowed ($file['name'])) {
...
# HU, CS Repository plugin support
            if (!repository_make_upload_directory($destination, true)) { //TODO maybe put this function here instead of moodlelib.php now.
                $this->status = false;
                return false;
            }
...
# HU, CS Repository plugin support
        if (!repository_file_exists($destination .'/'. $file['name'])) {
            return;
        }
...
function clam_handle_infected_file($file, $userid=0, $basiconly=false) {
    ...
    $delete = true;
# HU, CS Repository plugin support
    if (repository_file_exists($CFG->quarantinedir) && is_dir($CFG->quarantinedir) && is_writable($CFG->quarantinedir)) {
        $now = date('YmdHis');
...
# HU, CS Repository plugin support
    else if (repository_file_exists($file)) { // it's a path to somewhere on the filesystem!
        $fullpath = $file;
    }
    else {
...
# HU, CS Repository plugin support
    if (!$CFG->pathtoclam || !repository_file_exists($CFG->pathtoclam) || !is_executable($CFG->pathtoclam)) {
        $newreturn = 1;
