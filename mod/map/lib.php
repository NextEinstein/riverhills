...
function map_get_latlong_old(&$data){
	...
	$address = "http://maps.google.com/maps/geo?q=". str_replace(" ","+","$data->city,+$data->state,+$data->country") . "&output=xml&key={$CFG->map_google_api_key}";
...
function map_get_latlong_google(&$data,$address){
	global $CFG;
	$url = "http://maps.google.com/maps/geo?q=$address&output=xml&key={$CFG->map_google_api_key}";
