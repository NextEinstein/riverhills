<?php // $Id: config.php,v 1.1 2009/12/21 00:54:28 michaelpenne Exp $
...
$THEME->sheets = array('styles_menu', 'styles_page', 'styles_buttons');
...
/// Page theme specific configuration settings

$THEME->page_javascriptinclude = true;

/// When this is enabled (or not set!) then 
/// theme/page/lib.php:function page_theme_print_javacript() 
/// will try to include a file javascript.js from this theme.

$THEME->page_parentjavascriptinclude = false;

/// When this is enabled (or not set!) then 
/// theme/page/lib.php:function page_theme_print_javacript() 
/// will try to include a file javascript.js from 
/// this theme's parent (usually the page theme).

$THEME->page_onload = 'pageThemeRoundButtons(); pageThemeSetupMenu();';

// Sets menus to fan out on the left
if (get_string("thisdirection") == 'rtl' ) {
	$THEME->page_onload .= ' menusToRTL();';
}

/// This will be added to the body tag's onload attribute

$THEME->page_backtobutton = true;

/// When this is enabled (or not set!) then 
/// theme/page/lib.php:function page_theme_print_backto_button()
/// will attempt to print a back button to the page format

$THEME->page_signouttab = false;

/// When this is enabled (or not set!) then 
/// theme/page/lib.php:function page_theme_print_tabs()
/// will print a Sign out tab at the end of the header tabs

$THEME->page_menutab = true;

/// When this is enabled (or not set!) then 
/// theme/page/lib.php:function page_theme_print_tabs()
/// will print a menu tab at the start of the header tabs

// Modify theme's config for browswer white-list
if (isset($CFG)) {
    // config is set - we are not being called via smartpix script
    require_once($CFG->themedir.'/page/lib.php');
    page_theme_browser_whitelist($THEME);
}
